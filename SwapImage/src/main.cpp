...
private:
  ...
  void createLogicalDevice() {
    ...
    std::set<uint32_t> uniqueQueueFamilies = {
        indices.s_GraphicsFamily.value(),
        indices.s_PresentFamily.value(),
    };
    ...
    if (vkCreateDevice(m_PhysicalDevice, &createInfo, NULL, &m_Device) !=
        VK_SUCCESS) {
      throw std::runtime_error("ERROR::FAILED_TO_CREATE_LOGICAL_DEVICE");
    } else {
      std::cout << "SUCCESS::CREATED_LOGICAL_DEVICE" << std::endl;
    }
  }
};
